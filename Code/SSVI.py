...
def myconHess(x=None, v=None, phifun=None):
    ...
    if __switch_0__ == 'heston_like':
        ...
        c = np.array([[0, np.sign(rho)], [np.sign(rho), 0]]) #by construction, rho is the first parameter, lamda the second
    else:
        raise Exception('Incorrect function for phi')
    return c#(c, ceq)
...
def fit_svi(x0=None, k=None, 
    ...
    targetfun = lambda x: fitfunctionSVI(x, k,
                                         ...
                                         param_slice_before,
                                         param_slice_after)
    # only optimize first three variables, final two are set by no-arbitrage condition with the SSVI jumpwing parametrization
    # meaning of parameters x : [v, psi, p, c, vt]
    x0 = x0[:3]
    lb = lb[:3]
    ub = ub[:3]
    ...
    #                                               keep_feasible=False))
    constraints = {'type' : 'ineq', 
                   'fun' : lambda x : np.array(np.expand_dims(A, axis=0) @ x) ,
...
def interpolateGrid(df,
                    ...
                    S0,
                    bootstrap):
    reindexedDf = df.set_index(["Strike","Maturity"], drop=False)
    impliedVolInterpolated = pd.Series()
    #print(df.head())
    for smile in reindexedDf.rename({"Maturity" : "MaturityColumn"}, axis=1).groupby("MaturityColumn"):
        maturity = smile[0]
        tau_interp = maturity
...
class SSVIModel:
    ...
    def fit(self, df):
        filteredDf = removeMaturityInvalidData(df).set_index(["Strike", "Maturity"], drop=False)
        ...
        self.forward_theta = forward.groupby("Maturity").mean()
